...
sequenceDiagram
  ...
  App->>Tolka API: POST /v1/medical/session {domain, source_language, target_language}
...
const result = await tolka.translate({
  ...
  // source_language and target_language are optional when session_token is present
